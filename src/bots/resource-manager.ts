...
export async function handler(medplum: MedplumClient, event: BotEvent): Promise<Resource> {
  ...
  if (!ALLOWED_RESOURCE_TYPES.includes(input.resourceType)) {
    throw new Error(`Resource type ${input.resourceType as string} is not allowed`);
  }
  ...
  try {
    switch (input.type) {
      ...
      case 'delete':
        ...
        return medplum.deleteResource(input.resourceType, input.id) as Promise<Resource>;

      default:
        throw new Error(`Unsupported operation type: ${input.type as string}`);
    }
  } catch (error) {
